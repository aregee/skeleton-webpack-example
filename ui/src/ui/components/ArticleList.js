...
function selectPage(pageNumber) {
  var limit = getCurrentLimitFromArticles(domain.store.selectedView);
...
function view() {
  ...
  if (domain.store.selectedView.isLoading) {
    return m('div.article-preview', 'Loading...');
  }

  if (domain.store.selectedView.list.length === 0) {
    return m('div.article-preview', 'No articles are here... yet.');
  }

  totalPages = getTotalPages(domain.store.selectedView.limit, domain.store.selectedView.total);
  currentPage = getCurrentPage(domain.store.selectedView.limit, domain.store.selectedView.offset);

  return m('div', [
    domain.store.selectedView.list.map(function (article) {
      return m(ArticlePreview, {
        key: article.conversationId,
        article: article
      });
    }),
    m(Pagination, {
      ...
      fn_onItemClick: selectPage.bind(this)
    })
  ]);
};
